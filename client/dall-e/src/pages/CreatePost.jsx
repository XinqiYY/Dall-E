...
const CreatePost = () => {
  // nav back to home page once post has been created
  const navigate = useNavigate();
  const [form, setForm] = useState({ // store the form info
    name: '',
    prompt: '',
    photo: '',
  });
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-r8wt.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSumbit = async (e) => {
    // ensure the browser automatically reload the application
    e.preventDefault();
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://dall-e-r8wt.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <section className='max-w-7xl mx-auto'>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSumbit}>
        <div className='flex flex-col gap-5'>
          ...
          {/* showing image or generating img */}
